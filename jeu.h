...
void afficherTriangle(float ajoutY, float compteur, float ajoutX, bool var);
...
void finDuJeuFonct(Player tabToutPlayer[],int nbJoueur, int** finDuJeu, int indiceJoueur, ALLEGRO_SAMPLE_INSTANCE* musicFin);
void testPVDesJoueur(int indiceDuJoueur,Player tabToutPlayer[], int*** nbJoueurEnJeu, Cases tabPlateau[NB_CASES][NB_CASES], ALLEGRO_BITMAP* mort, ALLEGRO_EVENT event, ALLEGRO_BITMAP* FondJeu, ALLEGRO_BITMAP* map, int nbJoueur, int indicePourBitmap, Ligne ligne, int Iter, int Jter, ALLEGRO_SAMPLE_INSTANCE* musicmort);
void choixCaseDepart(Player tabPlayer[], int nbJoueur, PosDepart tabPosDepart[NBJOUEURMAX], Cases tabPlateau[NB_CASES][NB_CASES]);
